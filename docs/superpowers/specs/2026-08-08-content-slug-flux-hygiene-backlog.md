...
**Date:** 2026-08-08  
**Status:** Implemented in pipeline (PR closes #12) — legacy corpus slug cleanup remains ops  
...
| # | Issue | Status |
|---|---|---|
| 1 | Valid slug generation + cleanup | Generate: `make_slug()`; corpus redirects still ops |
| 2 | Consolidate Takeaways → **Key Takeaways** | Generate + repair + verify prompts |
| 8 | FLUX cover quality consistency | `IMAGE_STEPS=4` + slug-stable seed |
| 14 | SEO: answer-first opener + Key Takeaways high on page | Generate + repair prompts |
| 15 | SEO: question-style H2s + FAQ section in generate/repair | Generate + repair prompts |
...
**Done (new posts):** `generate_digest.make_slug()` caps length, strips trailing hyphens, collapses `--`. Legacy corpus rename/redirect remains a separate ops pass.
...
**Done:** Generate/repair/verify prompts require exactly one `## Key Takeaways`.
...
**Done:** House default `IMAGE_STEPS=4` (flux-1-schnell plateau). Editorial covers pass a **slug-stable seed** into `image_client.generate()`. Override via `IMAGE_STEPS` / `IMAGE_SEED` env.
...
**Done:** Prompt structure in `generate_digest.py` and `repair_prompts.py`.
...
**Done:** Prompt structure in generate + repair; verify preserves FAQ / heading hierarchy.
...
- One-time rename of historical bad slugs on `portfolio-blog` (ops).

## Suggested execution order (this repo)

1. Content structure: #2, #14, #15 (prompt + repair) — done  
2. Slug rules + corpus cleanup: #1 — generate done; corpus ops remaining  
3. FLUX consistency experiments: #8 — done (steps + seed policy)  
